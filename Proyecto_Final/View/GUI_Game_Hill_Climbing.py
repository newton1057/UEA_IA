...
class Main_Game:
    ...
    def seleccionar_opcion(self, ventana):
        ...
        if opcion_seleccionada == "- COMENZAR -":
            logging.info("Se ha comenzado el juego con el algoritmo Hill Climbing.")
            return True
        else:
            return False
...
class Game:
    ...
    def Encontrar_Posicion_Robot(self, Tablero : list):
        logging.info('Buscando la posición del robot en el tablero de juego.')
        for i in range(len(Tablero)):
            for j in range(len(Tablero)):
                if Tablero[i][j] == "R":
                    logging.info('Se ha encontrado la posición del robot en el tablero de juego en las coordenadas: ' + str(i) + ', ' + str(j) + '.')
                    return j, i

    def Encontrar_Posiciones_Obstaculos(self, Tablero : list):
        logging.info('Buscando la posición de los obstáculos en el tablero de juego.')
        Lista_Obstaculos = []
        for i in range(len(Tablero)):
            for j in range(len(Tablero)):
                if Tablero[i][j] == "*" and i != 0 and i != 11 and j != 0 and j != 11:
                    obstaculo = []
                    logging.info('Se ha encontrado la posición de un obstáculo en el tablero de juego en las coordenadas: ' + str(i) + ', ' + str(j) + '.')
    ...
    def press_failed(self, event):
        logging.error('¡El robot no a llegado a la Meta!')
    ...
    def on_enter(self, event):
        ...
        if Option == True:
            ...
            self.Implementation = Hill_Climbing(self.Game_Board)
            logging.info('Se ha iniciado el juego con el algoritmo Hill Climbing.')
            hilo = threading.Thread(target=self.Implementation.Search)
            logging.info('Se ha iniciado el algoritmo Hill Climbing en un subproceso aparte.')
            hilo.start()
            
    def run(self):
        logging.info('Se iniciara el juego con el algoritmo Hill Climbing.')
...
class Game_Board:
    ...
    def Motion_Up(self, x, y):
        logging.info('Se ha realizado un movimiento hacia arriba.')
    ...
    def Motion_Down(self, x, y):
        if self.Game_Board[y+1][x] == " ":
            logging.info('Se ha realizado un movimiento hacia abajo.')
            self.Game_Board[y][x] = " "
            self.Game_Board[y+1][x] = "R"
            Evaluacion = 1
    ...
    def Motion_Left(self, x, y):
        if self.Game_Board[y][x-1] == " ":
            logging.info('Se ha realizado un movimiento hacia la izquierda.')
            self.Game_Board[y][x] = " "
    ...
    def Motion_Right(self, x, y):
        if self.Game_Board[y][x+1] == " ":
            logging.info('Se ha realizado un movimiento hacia la derecha.')
            self.Game_Board[y][x] = " "
            self.Game_Board[y][x+1] = "R"
            Evaluacion = -3
            return True , Evaluacion
    ...
    def Motion_Suroeste(self, x, y):
        if self.Game_Board[y+1][x-1] == " ":
            logging.info('Se ha realizado un movimiento hacia el suroeste.')
            self.Game_Board[y][x] = " "
    ...
    def Motion_Sureste(self, x, y):
        if self.Game_Board[y+1][x+1] == " ":
            logging.info('Se ha realizado un movimiento hacia el sureste.')
            self.Game_Board[y][x] = " "
            self.Game_Board[y+1][x+1] = "R"
            Evaluacion = 1
            if self.Game_Board[y+2][x+1] == " ":
...
class Hill_Climbing:
    def __init__(self, Game_Board : Game_Board):
        logging.info('Se ha iniciado el algoritmo Hill Climbing en subproceso.')
        self.Game_Board = Game_Board
    ...
    def Evaluar_Estado(self, Evaluacion_Actual, Evaluacion_Siguiente):
        ...
        if Evaluacion_Actual > Evaluacion_Siguiente:
            ...
            logging.info('El estado actual del tablero de juego es peor que el estado siguiente.')
            return True
    ...
    def Algoritmo_Hill_Climbing(self, Evaluacion_Estado):
        logging.info('Se esta aplicando el algoritmo Hill Climbing.')
        logging.info('Evaluación del estado: ' + str(Evaluacion_Estado))
        if self.Validar_Solucion():
            logging.info('🤖 Robot llego a la meta!')
            pyautogui.press("S")
        else:
            pyautogui.press("R")
            ...
            for i in range(5):
                ...
                logging.info("Se realizo una copia del tablero de juego.")

                self.Game_Board.Game_Board = Game_Board_AUX
                
                Movimiento, Evaluacion = self.Movimientos(Movimientos_Realizados)

                if Movimiento is None and Evaluacion is None:
                    logging.error('El robot no puede llegar a la meta.')
                    logging.error('Se ha alcanzado el máximo local.')
                    pyautogui.press("F")
                    sys.exit()
                ...
                if (self.Evaluar_Estado(Evaluacion_Estado, Evaluacion_Aux)):
                    ...
                    self.Algoritmo_Hill_Climbing(Evaluacion_Estado)
                else:
                    Movimientos_Realizados.append(Movimiento)
                    self.Game_Board.Game_Board = Game_Board_Original
                    logging.warning("🤖 El robot no puede llegar a la meta.")
    
    def Search(self):
        self.Algoritmo_Hill_Climbing(0)
