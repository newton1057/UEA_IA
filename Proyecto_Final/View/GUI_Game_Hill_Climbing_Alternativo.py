...
class Main_Game:
    ...
    def seleccionar_opcion(self, ventana):
        ...
        if opcion_seleccionada == "- COMENZAR -":
            logging.info("Se ha comenzado el juego con el algoritmo Hill Climbing Alternativo.")
            return True
        else:
            return False
...
class Game:
        ...
        for i in range(len(Tablero)):
            for j in range(len(Tablero)):
                if Tablero[i][j] == "*" and i != 0 and i != 11 and j != 0 and j != 11:
                    obstaculo = []
                    logging.info('Se ha encontrado un obstaculo en la posicion: ' + str(j*40) + " " + str(i*40))
    ...
    def on_enter(self, event):
        ...
        if Option == True:
            ...
            self.canvas.destroy()
            self.Render()
            logging.info('Se ha iniciado el juego con el algoritmo Hill Climbing Alternativo.')
            self.Implementation = Hill_Climbing(self.Game_Board)
            hilo = threading.Thread(target=self.Implementation.Search)
            logging.info('Se ha iniciado el algoritmo Hill Climbing Alternativo en un subproceso aparte.')
            hilo.start()

    def run(self):
        logging.info('Se iniciara el juego con el algoritmo Hill Climbing Alternativo.')
...
class Game_Board:
    def __init__(self, Game_Board : list):
        ...
        self.Objetive_X = self.data['x_axis']
        self.Objetive_Y = self.data['y_axis']
        logging.info('Se ha creado el tablero de juego.')
...
class Hill_Climbing:
    def __init__(self, Game_Board : Game_Board):
        logging.info('Se ha iniciado el algoritmo Hill Climbing Alternativo en un subproceso.')
        self.Game_Board = Game_Board
        ...
        if self.Validar_Solucion():
            ...
            for i in range(5):
                ...
                if Movimiento is None and Evaluacion is None:
                    ...
                    logging.error('Maximo local alcanzado.')
                    pyautogui.press("F")
                    sys.exit()
                ...
                if (self.Evaluar_Estado(Evaluacion_Estado, Evaluacion_Aux)):
                    ...
                    logging.info('🤖 Robot aun no puede llegar a la meta!')
